...
*/
const SUPABASE_URL='https://cvawcwzxtmwoigczwmbq.supabase.co';
const SUPABASE_KEY='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImN2YXdjd3p4dG13b2lnY3p3bWJxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDY4NzY5NTUsImV4cCI6MjA2MjQ1Mjk1NX0.0VT311gL4VM4G808CgjBfl5MGgnX0paIiNLnw0dd0F0';
...
const crearCliente=(nombre,precio,descripcion)=>{
    ...
    .then(async(res)=>{
        if(!res.ok){
            const text=await res.text();//responde al error en texto de la tabla
            throw new Error(text || 'error al insertar productos');
        }
...
const actualizarCliente=(nombre,precio,descripcion,id)=>{
    return fetch(`${API_URL}?id=eq.${id}`,{
        method:'PATCH',
        headers:{
            ...HEADERS,
...
export const clientServiceProd = {
    listaclientes,
    crearCliente,
    eliminarCliente,
    clientes,
