...
const crear_nueva_fila=(nombre,precio,descripcion,id)=>{// recepciono datos 
    ...
    const contenido = `
        ...
        btn.addEventListener("click",()=>{
            const id=btn.id;
            clientServiceProd.eliminarCliente(id)
                .then(respuesta=>{
                alert("eliminado")
...
const table = document.querySelector("[data-tableProd]");
clientServiceProd
.listaclientes()
    .then((data)=>{
    data.forEach(({nombre,precio,descripcion,id}) => {
                const nuevaLinea= crear_nueva_fila(nombre,precio,descripcion,id)// llamo a 3 referencias
